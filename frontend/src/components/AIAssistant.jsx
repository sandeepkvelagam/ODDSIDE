...
export default function AIAssistant({ currentPage }) {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: "Hi! I'm your Kvitt assistant. Ask me anything — your groups, games, stats, settlements, or how the app works!"
    }
  ]);
    ...
    <Card className="fixed bottom-5 right-5 w-80 sm:w-96 h-[520px] flex flex-col bg-card border-border shadow-2xl z-50 rounded-xl overflow-hidden">
      ...
      {!hasStarted ? (
        ...
        <>
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  >
                    ...
                    {msg.role === 'user' ? (
                      <div className="max-w-[78%] px-3 py-2 rounded-2xl rounded-br-none text-sm bg-violet-600 text-white">
                          ...
                          <div className="px-3 pb-2 flex flex-wrap gap-1 border-t border-border/50 pt-2 mt-1">
                            {msg.followUps.map((s, j) => (
                              <button
                                ...
                                className="text-[10px] px-2 py-1 bg-background/60 hover:bg-secondary rounded-full text-muted-foreground hover:text-foreground transition-colors"
                              >
                                {s}
                              </button>
                            ))}
          ...
          {/* Initial suggestions — only shown for first couple messages */}
