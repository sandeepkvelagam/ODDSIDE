...
// TODO: Host this image on our own storage (S3/Supabase Storage) after migration
const POKER_HANDS_IMAGE = "/assets/poker-hand-rankings.webp";
...
export default function GameNight() {
  ...
  const [settlementCalcOpen, setSettlementCalcOpen] = useState(false);
  const [needsSettle, setNeedsSettle] = useState(false);
  ...
  const handleEndGame = async () => {
    ...
    try {
      ...
      setNeedsSettle(false); // Settlement auto-generated by backend
      setSettlementCalcOpen(true); // Open calculator modal
      fetchGame();
    } catch (error) {
  ...
  const handleSettle = () => {
    setNeedsSettle(true); // Needs to call settle API
    setSettlementCalcOpen(true); // Open calculator modal
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-8">
        ...
        <Dialog open={editChipsDialogOpen} onOpenChange={setEditChipsDialogOpen}>
          <DialogContent className="bg-card border-border mx-4 max-w-md">
            ...
            <div className="space-y-4 mt-4">
              ...
              {/* Warning for Settled Games */}
              {isSettled && (
                <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg">
                  <p className="text-sm text-amber-500 font-medium">⚠️ Settlement will be regenerated</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    This game is already settled. Editing chips will recalculate the settlement and notify all players.
                  </p>
                </div>
              )}
        ...
        <SettlementCalculator
          isOpen={settlementCalcOpen}
          onClose={() => setSettlementCalcOpen(false)}
          players={game?.players || []}
          chipValue={chipValue}
          gameId={gameId}
          gameTitle={game?.title}
          needsSettle={needsSettle}
          onComplete={() => navigate(`/games/${gameId}/settlement`)}
        />
      </main>
    </div>
  );
